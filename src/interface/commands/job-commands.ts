  ...
  if (runningTasks.length === 0 && queuedTasks.length === 0) {
    ...
    if (runningTasks.length > 0) {
      ...
      for (const task of runningTasks) {
        const duration = task.startedAt ? Math.round((Date.now() - task.startedAt) / 1000) : 0;
        const args = task.args.length > 50 ? task.args.substring(0, 50) + '...' : task.args;
        lines.push(`🆔 ${task.id}`);
...
export async function handleJobsShow(ctx: CommandContext): Promise<void> {
  ...
  const createdAt = new Date(task.createdAt).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' });
  lines.push(`创建时间: ${createdAt}`);

  if (task.startedAt) {
    const startedAt = new Date(task.startedAt).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' });
  ...
  if (task.completedAt) {
    const completedAt = new Date(task.completedAt).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' });
